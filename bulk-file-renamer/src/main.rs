...
fn main() {
    let matches = Command::new("Bulk File Renamer")
        ...
        .arg(
            Arg::new("with")
                .short('w')
                .long("with")
                .help("The replacement or addition to be made to the file names, can be skipped"),
        )
        .arg(
            Arg::new("quiet")
                .long("quiet")
                ...
                .help("Prevents logging of successful operations to the console"),
        )
        .get_matches();
    ...
    let error_msg = "Error: Invalid path, make sure it points to a valid, accessible directory";
    ...
    visit_dirs(path, &mut |entry: &PathBuf| {
        ...
        let new = match &modification {
            Modification::Replacement(r) => old.replace(r, &with),
            Modification::Start => {
                format!("{}{}", with, old)
            }
            Modification::End => {
        ...
        match (fs::rename(old_path, &new_path), quiet) {
            (Ok(_), false) => println!("Renamed: {:?} -> {:?}\n", old_path, new_path),
            (Ok(_), true) => (),
            (Err(err), _) => eprintln!(
                "\nFailed to rename {:?} -> {:?}: {}\n",
